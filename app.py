...
BREED_INFO = {
    ...
    "Beagle": {
        ...
        "characteristics": {
            "bred_for": "Hunting rabbits and hares",
            "breed_group": "Hound",
            "origin": "England"
        }
    },
    ...
    "Rottweiler": {
        ...
        "characteristics": {
            "bred_for": "Cattle driving, Guard dog",
            "breed_group": "Working",
            "origin": "Germany"
        }
    },
    ...
    "Labrador Retriever": {
        ...
        "characteristics": {
            "bred_for": "Retrieving game, Water retrieval",
    ...
    "Standard Poodle": {
        ...
        "description": "The Standard Poodle is known for its intelligence, elegant appearance, and versatility. Despite their association with luxury, they were originally bred as water retrievers.",
        "category": "non-sporting",
        "size": "large",
        "similar_breeds": ["Portuguese Water Dog", "Afghan Hound", "Kerry Blue Terrier"],
        "characteristics": {
            "bred_for": "Water retrieval, hunting",
            "breed_group": "Non-sporting",
...
def fetch_breed_info_from_api(breed_name):
    ...
    search_variations = [
        breed_name.lower(),  # original in lowercase
        breed_name.lower().replace(' ', ''),  # no spaces
        breed_name.lower().replace(' ', '-'),  # hyphenated
        breed_name.lower().replace(' ', '/'),  # with slashes
        ' '.join(word for word in breed_name.lower().split() if word not in ['standard', 'miniature', 'toy'])  # without size prefix
    ]
    ...
    while retry_count < max_retries:
        try:
            ...
            for search_query in search_variations:
                search_url = f"https://api.thedogapi.com/v1/breeds/search?q={search_query}"
                response = requests.get(search_url, headers=headers, timeout=10)
                response.raise_for_status()
                breeds = response.json()
                
                if breeds:  # If we found any breeds
                    breeds_data = breeds
            ...
            breed_info = breeds_data[0]  # Get the first matching breed
...
def get_breed_info(breed_name):
    ...
    variations = [
        ...
        clean_name.replace('Standard ', ''),
        clean_name.replace('Miniature ', ''),
        clean_name.replace('Toy ', ''),
    ]
